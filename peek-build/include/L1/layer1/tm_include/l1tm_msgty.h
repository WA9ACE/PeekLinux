...
/**************** ENUMs ***********************/
  ...
  enum ETMCommonTargetErrors {
      ...
      ETM_RV_NOT_SUPPORTED = -49,   // Funtionality not supported by RÍV
...
enum 
{
  ...
  #if (L1_TPU_DEV == 1)
    ,FLEXI_TPU_TABLE_WRITE   = 0x76,
     FLEXI_TPU_TABLE_READ     = 0x77,

  //Flexi ABB Delays
     FLEXI_ABB_WRITE   = 0x78,
     FLEXI_ABB_READ     = 0x79
  #endif
// DRP Calib
    ,DRP_CALIB_WRITE = 0x80,
    DRP_CALIB_READ = 0x81    
...
enum RF_PARAM 
{
  ...
  INITIAL_AFC_DAC    = 10
    ...
    ,REPEATED_SACCH_ENA_FLAG  =  21
  #endif  /* (FF_REPEATED_SACCH == 1) */

  #if (FF_REPEATED_DL_FACCH == 1)  
    ,REPEATED_FACCHDL_ENA_FLAG    =60
...
enum RF_TABLE 
{
  ...
  #endif
#if ((REL99 == 1) && (FF_BHO == 1))
   ,TX_LEVELS_POWER_REDUCTION      = 99,
    TX_LEVELS_POWER_REDUCTION_8PSK = 100
#endif
};
...
enum DRP_CALIB_ROUTINE
{
   DRP_DUMMY = 1, // Dummy Routine
   DRP_READ_FROM_FLASH = 2,
   DRP_DCXO_CALIB = 3,
   DRP_TX_RX_COMMON= 4,
   DRP_LNA_CFREQ = 5,
   DRP_IQMC= 6,
   ...
   DRP_AFE_GAIN_1 =9,
   DRP_AFE_GAIN_2 = 10,
   DRP_WRITE_TO_FLASH = 11
  
};

enum  SP_PARAM 
{
   SP_SLEEPDISABLE = 1, // SLEEP DISABLE
   SLEEPMODE = 2
};
