...
//#ifdef R2D_ISAMPLE_USE_MUXING
   ...
   #define  F_LCD_SET_PIN_CONFIG \
	...
	#define LCD_DATA_PORT	(*(volatile unsigned short*)0x02000000)
	#define RS_GPIO			44
	#define RESET_GPIO		43

	#define SET_RS() \
		GPIO_SET_OUTPUT(RS_GPIO)
		
	#define CLEAR_RS() \
		GPIO_CLEAR_OUTPUT(RS_GPIO)

	#define WRITE_LCD_CMD(cmd)	\
		CLEAR_RS(); \
		LCD_DATA_PORT = cmd

	#define WRITE_LCD_DATA(data) \
		SET_RS(); \
		LCD_DATA_PORT = data
		
	#define WRITE_LCD_CMDDATA(cmd, data) \
		WRITE_LCD_CMD(cmd); \
		WRITE_LCD_DATA(data)
	#define LCD_320x240


extern UINT32 lcd_g_sleep_delay;	//in ms
