...
#define FFS_SEEK_OPT			0
#if (LOCOSTO_LITE == 1)
	#define FFS_READ_CACHE_ENBALE 	0
#else
	#define FFS_READ_CACHE_ENBALE 	1
#endif

#define DL_GC_TIMER  1


#define FFS_PERFORMANCE_MEASURE  0
/******************************************************************************
 * Types
 ******************************************************************************/
...
typedef int32 location_t;  // object location offset
...
#else
    iref_t thisss;   // iref of dir that was opened
#endif
...
struct ffs_file_cnf_s {
	...
	UINT16	  op_id;	  // Requested operation id	    
#endif
...
enum FFS_ERRORS {
    ...
    EFFS_FWBUF2BIG   = -33, /* Flash write buffer to big */
    ...
    EFFS_NOREPINODE   = -44, /* No replacement inode found */
    EFFS_INVSEGHEAD   = -45, /* Invalid segment head! */
    EFFS_INVINODE     = -46, /* Found invalid inode */
    EFFS_2MANYREPI    = -47, /* To many replacement inodes */
    EFFS_JNLPUSH2DPTH = -48, /* Journal_push() to depth */
    EFFS_JNLPOP2DPTH  = -49, /* Journal_pop() to depth */
    EFFS_JNLFULL      = -50  /* Journal fill is full */
};
...
T_FFS_RET ffs_set_attr(const char *pathname, unsigned short attr);
...
extern fd_t lfs_open(const char *pathname, ffs_options_t option, unsigned int *size_of_file);
extern int lfs_write(fd_t fdi, void *src, int amount);
extern effs_t lfs_close(fd_t fdi);
extern req_id_t lfs_read(fd_t fdi, char **src);
extern effs_t lfs_remove(const char *pathname);
#endif
