...
int etm_pwr(uint8 *buf, int insize)
{
	...
	case PWR_DYNAMIC :
	    ...
	    *(buf+2) = 0xBA;
	    ttr((uint32) TTrAll,"*PWR Module Version = 0x%x" NL, PWRVERSION );
	    ttr((uint32) TTrAll,"*bat_id = %d" NL,               pwr_cfg->data.bat_id );
	    ttr((uint32) TTrAll,"*state = %d" NL,                pwr_ctrl->state);
	    ttr((uint32) TTrAll,"*chg_id = %d" NL,               pwr_cfg->data.chg_id );
	    ttr((uint32) TTrAll,"*Vbat = %d" NL,                 pwr_cfg->data.Vbat);
	    ttr((uint32) TTrAll,"*Vbat_avg = %d" NL,             pwr_cfg->data.Vbat_avg);
	    ttr((uint32) TTrAll,"*Vbat_avg_mV = %d" NL,          pwr_cfg->data.Vbat_avg_mV);
	    ttr((uint32) TTrAll,"*Tbat = %d" NL,                 pwr_cfg->data.Tbat);
	    ttr((uint32) TTrAll,"*Tbat_avg = %d" NL,             pwr_cfg->data.Tbat_avg);
	    ttr((uint32) TTrAll,"*Vchg = %d" NL,                 pwr_cfg->data.Vchg);
	    ttr((uint32) TTrAll,"*Ichg = %d" NL,                 pwr_cfg->data.Ichg);
	    ttr((uint32) TTrAll,"*Cbat = %d" NL,                 pwr_cfg->data.Cbat);
	    ttr((uint32) TTrAll,"*cfg_id = %d" NL,               pwr_cfg->data.cfg_id);
	    ttr((uint32) TTrAll,"*chg_cfg_id = %d" NL,           pwr_cfg->data.chg_cfg_id);
	    ttr((uint32) TTrAll,"*bforce = %d" NL,               pwr_cfg->data.bforce);
	    ttr((uint32) TTrAll,"*cforce = %d" NL,               pwr_cfg->data.cforce);
	    ttr((uint32) TTrAll,"*k = %d" NL,                    pwr_cfg->data.k);
	    ttr((uint32) TTrAll,"*T4 = %d" NL,                   pwr_cfg->data.T4);
	    ttr((uint32) TTrAll,"*T1 elapsed = %d" NL,           pwr_ctrl->time_elapsed_T1);
	    ttr((uint32) TTrAll,"*T2 elapsed = %d" NL,           pwr_ctrl->time_elapsed_T2);
	    ttr((uint32) TTrAll,"*T3 elapsed = %d" NL,           pwr_ctrl->time_elapsed_T3);
	    ttr((uint32) TTrAll,"*MMI timer elapsed = %d" NL,    pwr_ctrl->time_elapsed_mmi_rep);
	    error = PWR_OK;
	    break;
	case PWR_TRACE_MASK:
