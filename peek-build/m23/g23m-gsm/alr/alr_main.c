    ...
    case MA_CELL_RESELECTION:
      switch(idle->mod)
      {
        ...
        case MODE_CELL_RESELECTION:
          ...
          ma_stop_active_procs(STOP_PCH_READING);/* don´t forget to stop all processes */
#ifdef GPRS
          gprs_alr_init();
#endif
          alr_data->bs_pa_mfrms = idle->bs_pa_mfrms;
...
{
/* Implements Measure#32: Row 112 */
  GET_INSTANCE_DATA;
  TRACE_ASSERT((GET_STATE(STATE_MA) NEQ MA_NULL) OR (GET_STATE(STATE_MA) NEQ MA_CELL_SELECTION));
