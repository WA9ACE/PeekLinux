...
#ifdef FF_PS_RSSI
EXTERN UBYTE RX_GetRxLevAccessMin (void);
#endif
...
LOCAL void RX_Indication (void)
{
  if (rx_signal_callback)
  {
    ...
    new_rx_qual_value  = RX_GetRxQual();

    SYST_TRACE_P ((SYST, "RX_Indication(): rx=%u new, %u old", 
      calculated_level, rx_Status.actLevel));
    
    if ((calculated_level NEQ rx_Status.actLevel) OR 
        (new_rx_qual_value NEQ rx_Status.rxQuality))      
    {
      ...
      rx_Status.rxQuality       = new_rx_qual_value;
#ifdef FF_PS_RSSI
      rx_Status.min_access_level = RX_GetRxLevAccessMin ();
#endif
...
#ifdef FF_PS_RSSI
  out_StatusPtr->min_access_level = rx_Status.min_access_level;
#endif
