...
+-----------------------------------------------------------------------------
	...
	Nov 06, 2006 DR: OMAPS00102380 x0pleela	
	Description: Cannot build bt8 variant with fix for OMAPS00097714
	Solution: Added the MFW flag where call_data_tty and FFS_flashData are used in functions 
			audio_set_tty, audio_dyn_set_tty, audio_save_def_return
	...
	Solution: For the Loocsto-lite variant, when a TTY call is initiated, the 
                    ...
                    profile is loaded.
 	Apr 9, 2006    REF : DR OMAPS00071222     x0039928
 	Description:	CT-GCF[27.22.4.5]-Tones not generated
 	Solution:	Tone description table is updated with proper values for tones error, congest
 	RP Ack and RP not ack.
...
#ifdef FF_TTY
static T_AUDIO_TTY_CONFIG_PARAMETER tty_cfg;/*a0393213 18 Aug 2006 warning removal - storage class is not first*/
static UBYTE tty_state = FALSE;
static UBYTE tty_cmd = (UBYTE)TTY_OFF;
...
#endif
#endif
//x0pleela 06 Nov, 2006 DR: OMAPS102380
//Adding MFW flag also
/*x0039928 OMAPS00097714 HCO/VCO option - enum added for tty status and tty type*/
#if defined (FF_TTY_HCO_VCO) && defined (MFW)
typedef enum
{
...
//xdeepadh, nekkareb - OMAPS00076996: 05 May 2006
/* Added MFW flag for successful ACI only build */
#if defined (MMI_LITE) && defined (MFW)
extern int mfw_get_current_audioDevice();
extern char aud_cg_file[5][15]; //externalised to maintain sanity with MFW
int old_audioDevice;   //Variable to store the current audio profile
#endif /* MMI_LITE && MFW */
...
// Apr 9, 2006    REF : DR OMAPS00071222     x0039928
// Fix : Tone description table is updated with proper values.
...
int audio_set_device(char* mode)
{
	...
	T_AUDIO_MODE_LOAD p_parameter;
        ...
        convert_u8_to_unicode(mode, p_parameter.audio_mode_filename);
...
void audio_set_path(int audioDevice,int stereo)
{
	...
	audio_full_access_write(&p_parameter,return_path);
...
//	local variable declarations 
	...
	INT16 side_tone = AUDIO_SIDE_TONE;/*a0393213 18th aug 2006-warning removal-INT8 changed to INT16*/
#endif	
...
// Jun 02, 2006  DRT OMAPS00079213   x0039928(sumanth)
       INT16 status;
#ifdef FF_MMI_FILEMANAGER
	T_AUDIO_MODE_LOAD handheld;
	char * handheld_ascii = {"handheld"};
	convert_u8_to_unicode(handheld_ascii, handheld.audio_mode_filename);
#else
...
//	and the defualt mode handheld is loaded
	...
	status = Bsptwl3029_audio_accessory_register_notification(audioConfig[CALLBACK_HEADSET],(T_AUDIO_ACCESSORY_TYPE)AUDIO_ACCESSORY_HEADSET);
	if( status != AUDIO_OK)
	{
...
//	set the sidetone value to 175 this is the optimum value    
    	...
    	audio_full_access_write(&side_tone_cfg, return_path);
#else    	
	    ABB_SideTone (175);
#endif	    
    }
...
// Jun 02, 2006  DRT OMAPS00079213   x0039928(sumanth)
// Fix : Two new functions added to call BSP register and unregister API with 
// AUDIO_ACCESSORY HOOK as the parameter
#ifdef FF_MMI_AUDIO_PROFILE
void hook_unregister(void)
{
	int status;
	status = Bsptwl3029_audio_accessory_Unregister_notification(AUDIO_ACCESSORY_HOOK);
	if( status != AUDIO_OK)
	{
		TRACE_FUNCTION("Error - unregistering the callback for hook detect");
	}
}

void hook_register(T_ACCESSORY_CALLBACK audioConfig)
{
	int status;
	status = Bsptwl3029_audio_accessory_register_notification(audioConfig,(T_AUDIO_ACCESSORY_TYPE)AUDIO_ACCESSORY_HOOK);
	if( status != AUDIO_OK)
	{
			TRACE_FUNCTION("Error - registering the callback for hook detect");
	}
}
#endif
...
//	local variable 
	...
	T_RV_RETURN_PATH          	return_path = {0, NULL};
	T_AUDIO_RET					result;
	T_AUDIO_SPEAKER_LEVEL		volume;/*a0393213 18th aug 2006-warning removal-INT8 changed to T_AUDIO_SPEAKER_LEVEL*/
#endif	
...
//	The ABB values are converted to the corresponding audio service values
	...
	switch (in_Amplf)
	{
		...
		break;
		case 190:
		volume.audio_speaker_level = AUDIO_SPEAKER_VOLUME_6dB;
		break;
		case 125:
		volume.audio_speaker_level = AUDIO_SPEAKER_VOLUME_12dB;
		break;
		case 60:
...
#ifdef FF_MMI_FILEMANAGER
	convert_u8_to_unicode(audio_mode_names_tty[0], audio_mode.audio_mode_filename);
#else
      ...
      }
      {
        switch(tty_cmd)
        {
          case TTY_ALL:
            {
              tty_cfg.tty_mode = (UBYTE)TTY_API_ALL;
            }
            break;
          case TTY_VCO:
            {
              tty_cfg.tty_mode = (UBYTE)TTY_API_VCO;
            }
            break;
          case TTY_HCO:
            {
              tty_cfg.tty_mode = (UBYTE)TTY_API_HCO;              
            }
            break;
          default:
            {
              tty_cfg.tty_mode = (UBYTE)TTY_API_NOTTY;              
            }
            break;
        }
      }
#else //For Locosto lite(MMI_LITE)

//x0pleela 06 Nov, 2006 DR: OMAPS102380
//Adding MFW flag also
/*x0039928 HCO/VCO option - setting tty_cmd from FFS_flashData (for always option) or  ttyPfType (for next call option)*/

#if defined (FF_TTY_HCO_VCO) && defined (MFW)
FFS_ttyAlwaysOn = get_ffs_tty_status();
FFS_ttyPfType = get_ffs_tty_pftype();
calldata_tty = get_call_data_tty();
if(FFS_ttyAlwaysOn == FALSE && calldata_tty == CALL_TTY_ONNEXTCALL)
{
	if(ttyPfType == TTY_ONLY_PF)
		tty_cmd = (UBYTE)TTY_ALL;
	else if(ttyPfType == TTY_VCO_PF)
		tty_cmd = (UBYTE)TTY_VCO;
	else if(ttyPfType == TTY_HCO_PF)
		tty_cmd = (UBYTE)TTY_HCO;
}
else if(FFS_ttyPfType == TTY_ONLY_PF)
	tty_cmd = (UBYTE)TTY_ALL;
else if(FFS_ttyPfType == TTY_VCO_PF)
	tty_cmd = (UBYTE)TTY_VCO;
else if(FFS_ttyPfType == TTY_HCO_PF)
	tty_cmd = (UBYTE)TTY_HCO;
#else
     //Save the TTY command
     tty_cmd = (UBYTE)tty;
#endif
     {
       switch(tty_cmd)
    ...
    // Get the current audio device, so that we can 
    // restore in audio_tty_return function
    #ifdef MFW /* Added MFW flag for successful ACI only build */
    old_audioDevice = mfw_get_current_audioDevice();
    #endif /* MFW */
    
    ret.callback_func = audio_load_return;

#ifdef FF_MMI_FILEMANAGER
	convert_u8_to_unicode(audio_mode_names_tty[(int)tty_cmd], audio_mode.audio_mode_filename);
#else
    ...
    {
       AUDIO_TRC_FUNC ("AUDIO MODE LOAD success");
     }
#endif  //MMI_LITE
    }
    else
...
//x0pleela 06 Nov, 2006 DR: OMAPS102380
//Adding MFW flag also
/*x0039928 HCO/VCO option - setting cmd from FFS_flashData (for always option) or  ttyPfType (for next call option)*/
#if defined (FF_TTY_HCO_VCO) && defined (MFW)
FFS_ttyAlwaysOn = get_ffs_tty_status();
FFS_ttyPfType = get_ffs_tty_pftype();
calldata_tty = get_call_data_tty();
...
#ifdef FF_MMI_FILEMANAGER
	convert_u8_to_unicode(audio_mode_names_tty[(int)cmd], audio_mode.audio_mode_filename);
#else
...
LOCAL void audio_tty_return (void *tty_result)
{
  ...
  ret.callback_func = audio_load_return;
  //xdeepadh, nekkareb - OMAPS00076996: 05 May 2006
  //Restore the audio profile back
#ifndef MMI_LITE  
#ifdef FF_MMI_FILEMANAGER
   convert_u8_to_unicode(audio_mode_names_tty[0], audio_mode.audio_mode_filename);
#else
...
#else //(MMI_LITE)
  #ifdef MFW /* Added MFW flag for successful ACI only build */
  strcpy (audio_mode.audio_mode_filename, (char *)aud_cg_file[old_audioDevice]);
  #endif /* MFW */
#endif //MMI_LITE
  audio_ret = audio_mode_load (&audio_mode, ret);
