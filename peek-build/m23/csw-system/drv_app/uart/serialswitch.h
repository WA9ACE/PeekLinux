...
// uncomment only for testing of the new aGPS CSMI streamport
//#define TEST_AGPS_CSMI_PORT 
...
#else
  void SER_InitSerialConfig (T_AppliSerialInfo *serial_info);
#endif //DP

SYS_BOOL SER_UartSleepStatus (void);

void SER_WakeUpUarts (void);
...
SYS_UWORD32 SER_tr_ReadNChars (int serial_data_flow,
                               char *buffer,
                               SYS_UWORD32 chars_to_read);

SYS_UWORD32 SER_tr_ReadNBytes (int serial_data_flow,
                               char *buffer,
                               SYS_UWORD32 chars_to_read,
                               SYS_BOOL *eof_detected);

SYS_UWORD32 SER_tr_WriteNChars (int serial_data_flow,
                                char *buffer,
                                SYS_UWORD32 chars_to_write);

SYS_UWORD32 SER_tr_EncapsulateNChars (int serial_data_flow,
                                      char *buffer,
                                      SYS_UWORD32 chars_to_write);

SYS_UWORD32 SER_tr_WriteNBytes (int serial_data_flow,
                                SYS_UWORD8 *buffer,
                                SYS_UWORD32 chars_to_write);

void SER_tr_WriteChar (int serial_data_flow,
                       char character);

SYS_BOOL SER_tr_EnterSleep (int serial_data_flow);

void SER_tr_WakeUp (int serial_data_flow);

void SER_tr_WriteString (int serial_data_flow,
                         char *buffer);
...
#if (DP==0)
  T_FDRET SER_fd_Initialize (void);
#endif

T_FDRET SER_fd_Enable (SYS_BOOL enable);

T_FDRET SER_fd_SetComPar (T_baudrate baudrate,
                          T_bitsPerCharacter bpc,
                          T_stopBits sb,
                          T_parity parity);

T_FDRET SER_fd_SetBuffer (SYS_UWORD16 bufSize,
                          SYS_UWORD16 rxThreshold,
                          SYS_UWORD16 txThreshold);

T_FDRET SER_fd_SetFlowCtrl (T_flowCtrlMode fcMode,
                            SYS_UWORD8 XON,
                            SYS_UWORD8 XOFF);

T_FDRET SER_fd_SetEscape (char escChar,
                          SYS_UWORD16 guardPeriod);

T_FDRET SER_fd_InpAvail (void);

T_FDRET SER_fd_OutpAvail (void);

T_FDRET SER_fd_EnterSleep (void);

T_FDRET SER_fd_WakeUp (void);

T_FDRET SER_fd_ReadData (T_suspendMode suspend,
...
T_FDRET SER_fd_WriteData (T_suspendMode suspend,
...
T_FDRET SER_fd_StopRec (void);

T_FDRET SER_fd_StartRec (void);

T_FDRET SER_fd_GetLineState (SYS_UWORD32 *state);

T_FDRET SER_fd_SetLineState (SYS_UWORD32 state,
                             SYS_UWORD32 mask);

#if (DP==0)
  T_FDRET SER_fd_CheckXEmpty (void);
#endif

#ifdef BTEMOBILE
  T_HCI_RET SER_bt_Init (void);

  T_HCI_RET SER_bt_Start (void);

  T_HCI_RET SER_bt_Stop (void);

  T_HCI_RET SER_bt_Kill (void);

  T_HCI_RET SER_bt_SetBaudrate (UINT8 baudrate);

  T_HCI_RET SER_bt_TransmitPacket (void *uart_sco_tx_buffer);
  ...
  void SER_bt_WakeUp (void);
   ...
   void SER_uart_handler (void);
#elif ((CHIPSET == 4) || (CHIPSET == 5) || (CHIPSET == 6) || (CHIPSET == 7) || (CHIPSET == 8) || (CHIPSET == 9) || (CHIPSET == 10) || (CHIPSET == 11) || (CHIPSET == 12)|| (CHIPSET == 14))
   void SER_uart_modem_handler (void);
   void SER_uart_irda_handler (void);
#elif (CHIPSET == 15)
   void SER_uart_irda_handler (void);
#endif
#if (CHIPSET == 12)
   void SER_uart_modem2_handler (void);
#endif
