...
#if (defined (BTEMOBILE) && (CHIPSET != 12) && (CHIPSET !=15))
  #include "ffs/ffs.h" 
...
#if (defined (RVM_USBTRC_SWE) && (TRACE_ON_USB == 1))
...
#define DISABLE_WAKE_UP_INTERRUPT(UART) \
            *((volatile SYS_UWORD8 *) ((UART)->base_address + SCR*SEPERATOR)) &= \
 ...
 */
#if (UARTFAX_STATE == 1 || UARTFAX_STATE == 2 ||CHIPSET != 15)
...
#endif /* (defined BTEMOBILE && (CHIPSET != 12)&& (CHIPSET !=15)) */
...
typedef struct s_uart {
    ...
    SYS_WORD16  flow_id;
    void (*interrupt_handler) (T_tr_UartId uart_id,
...
#elif (CHIPSET == 15)
  SYS_UWORD32 uart_irda_spurious_interrupts;
#endif
                    ...
                    T_SerialDriver serial_driver)
{
    ...
    case UART_MODEM_TRACE:
      ...
      case UART_MODEM2_TRACE:
        ...
        if (serial_driver == UART_IRDA_TRACE)
		  ...
		  #endif
		  /* Shouldn't Come Here for CHIPSET==15 */
        }
	...
	int_uart[tr_functions[flow]->device].interrupt_handler =
		UA_InterruptHandler;
#elif (defined (RVM_USBTRC_SWE) && (TRACE_ON_USB == 1))
	int_uart[tr_functions[flow]->device].interrupt_handler =
...
SER_InitSerialConfig (T_AppliSerialInfo *serial_info)
{
    ...
    while (flow < SER_MAX_NUMBER_OF_FLOWS) {
        ...
        set_flow_functions (flow,(T_SerialDriver)serial_driver);
        flow++;
    }
...
#if (defined (RVM_USBTRC_SWE) && (TRACE_ON_USB == 1))
            USBT_initialize_uart_sleep ((T_tr_UartId)uart_id);
#else
...
SER_ImmediateSwitch (void)
{
    ...
    #if ((CHIPSET == 4) || (CHIPSET == 5) || (CHIPSET == 6) || (CHIPSET == 7) || (CHIPSET == 8) || (CHIPSET == 9) || (CHIPSET == 10) || (CHIPSET == 11)|| (CHIPSET == 14))
    ...
    rvf_disable (); /* beginning of the critical section */
...
#if (defined (RVM_USBTRC_SWE) && (TRACE_ON_USB == 1))
                    USBT_initialize_uart_sleep (uart_nb);
#else
...
#elif ((CHIPSET == 4) || (CHIPSET == 5) || (CHIPSET == 6) || (CHIPSET == 7) || (CHIPSET == 8) || (CHIPSET == 9) || (CHIPSET == 10) || (CHIPSET == 11) || (CHIPSET == 12)|| (CHIPSET == 14) || (CHIPSET == 15))
...
UT_Init (int device_id,
         ...
         void (callback_function (void)))
{
    SER_tr_Init (SER_PROTOCOL_STACK, (T_tr_Baudrate)baudrate, callback_function);
}
