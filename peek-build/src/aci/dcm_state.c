...
*                   -DCM Result : DCM_OK, DCM_NOT_READY, DCM_UNKNOWN_EVENT
*******************************************************************************/
EXTERN  BOOL Is_deactivate_ind_ing  ;
...
T_DCM_RET dcm_closing_conn(T_DCM_HDR * msg_p )
{
  ...
  switch(dcm_env_ctrl_blk_p->substate[0])
  {
	...
	//pinghua add one case on here.
       	 if(Is_deactivate_ind_ing==TRUE)
        	{
              	 Is_deactivate_ind_ing =FALSE ;
       	        return dcm_process_cgdeact_ind(msg_p, current_row);				
        	}
	 	else 
