...
/*==== VARIABLES ==================================================*/
/*==== FUNCTIONS ==================================================*/
LOCAL void io_DTIPSIsendString (UBYTE *string, USHORT string_len,
...
GLOBAL void psi_src_result_cb (UBYTE             src_id,
                                T_ATI_OUTPUT_TYPE	output_type,
                                UBYTE             *output,
                                USHORT            output_len)
{
  T_ACI_DTI_PRC_PSI *src_infos;
  TRACE_FUNCTION("psi_src_result_cb()");

  src_infos = find_element (psi_src_params, (UBYTE)src_id, cmhPSItest_srcId);
  if (src_infos EQ NULL)
  {
    TRACE_EVENT_P1("[ERR] psi_src_result_cb: src_id=%d not found", src_id) ;
    return ;
  }
  ...
  io_DTIPSIsendString (output, output_len, src_infos, output_type);  
}


GLOBAL void psi_src_line_state_cb (UBYTE                 src_id,
                                    T_ATI_LINE_STATE_TYPE	line_state_type,
                                    ULONG                 line_state_param)
{
  T_ACI_DTI_PRC_PSI   *src_infos;

  TRACE_FUNCTION("psi_src_line_state_cb()");

  src_infos = find_element (psi_src_params, (UBYTE)src_id, cmhPSItest_srcId);
  if (src_infos EQ NULL)
  {
    TRACE_EVENT ("[ERR] psi_src_line_state_cb: src_id not found");
    return;
  }
  ...
  switch (line_state_type)
  {
    ...
    case ATI_LINE_STATE_DCD:
      ...
      {
        BITFIELD_SET (src_infos->data_cntr, PSI_DTI_SB_BIT);
      }
      else
      {
        BITFIELD_CLEAR (src_infos->data_cntr, PSI_DTI_SB_BIT);
      }
   
      psaPSI_DCDreq( src_id, (UBYTE)line_state_param );
    ...
    case ATI_LINE_STATE_RNG:
    {
      ...
      psaPSI_RINGreq( src_id, (UBYTE)rng_params.ring_stat );
      break;
    }
    ...
    default:
    {
      TRACE_EVENT_P1("[WRN] psi_src_line_state_cb (): "\
...
GLOBAL void psi_buffer_data (T_ACI_DTI_PRC_PSI *src_infos, T_desc2 *send_data)
{
  T_desc2 *active_buffer, *last_buffer;
  USHORT i, j;
  T_TIME current_time;
  TRACE_FUNCTION("psi_buffer_data()");
  ...
  {
    /*
        If no device is connected to PSI it is possible that the buffer
...
//    if (src_infos->LineState EQ LINE_CONNECTED) // line is always connected
    {
      ...
      {
        TRACE_EVENT_P1 ("[WRN] psi_buffer_data(): life time expired, buffer deleted (%d)",
  ...
  while (i < send_data->len)
  {
    ...
    if (j>1)
    {
      ...
      memcpy((char *)&active_buffer->buffer[active_buffer->len], (char *)&send_data->buffer[i], j);
      active_buffer->len += j;
      i += j;
    }
    else
...
GLOBAL void psi_send_buffer_data (UBYTE src_id)
{
  T_ACI_DTI_PRC_PSI   *src_infos = NULL;

  TRACE_FUNCTION("psi_send_buffer_data()");

  /* dti_id should be supported by DTI_ENTITY_ID or something, here.. */
  src_infos = find_element (psi_src_params, src_id,
                            cmhPSItest_srcId);
  ...
  if (src_infos->data_buffer.data_list NEQ NULL)
  {
#ifdef _SIMULATION_
    {
      ...
      if(get_source_type(src_infos->srcId, &src_type))
      {
        ...
        {
          psa_psi_DTI_data_req (src_infos->data_buffer.data_list, src_infos, DTI_ENTITY_PSI);
        }
      }
    }
#else
...
|                                 ROUTINE : io_DTIPSIsendString        |
+-------------------------------------------------------------------+

  PURPOSE : format output string and send to the PSI through DTI
*/
                             ...
                             T_ATI_OUTPUT_TYPE output_type)
{
  ...
  TRACE_FUNCTION("io_DTIPSIsendString");
  ...
  {
    TRACE_ERROR ("String too large for io_DTIPSIsendString!");
        ...
        (aci_src_dti_params[src_infos->srcId].dtxState EQ READY))
  {
    ...
    {
      psa_psi_DTI_data_req (send_data, src_infos, DTI_ENTITY_PSI);
    }
#else
  ...
  {
    /* buffer these datas */
    psi_buffer_data (src_infos, send_data);
    ACI_MFREE (send_data);
  }
}
...
/*
+-------------------------------------------------------------------+
| PROJECT : GSM-PS (6147)         MODULE  : ATI_SRC_PSI_IO_C       |
...
+--------------------------------------------------------------------+
| PROJECT : GSM-PS (6147)         MODULE  : ATI_SRC_PSI_IO_C        |
|                                 ROUTINE : psi_src_test_sendString |
+--------------------------------------------------------------------+
  ...
  PURPOSE : Provides the io_DTIsendString() for the test-source-callback
            ...
            case of buffering messages during PSI is not connected
            with ACI
*/
GLOBAL void psi_src_test_sendString (UBYTE *string, USHORT string_len,
                             T_ACI_DTI_PRC_PSI     *src_infos,
                             T_ATI_OUTPUT_TYPE output_type)
{
  UBYTE *tmp;
  ...
  io_DTIPSIsendString(tmp, (USHORT) (string_len + 1), src_infos, output_type);
