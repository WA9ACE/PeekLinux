...
//EXTERN T_DCM_RET dcm_new_state(U8 state, U8 substate) ;
BOOL Is_deactivate_ind_ing = FALSE;
    ...
    switch ( state )
    {
      ...
      case PDP_CONTEXT_STATE_ACTIVATED:
        ...
        if ( is_ip_dti_id(p_pdp_context_node->internal_data.entity_id) )
        {
		  ...
		  p_prev_pdp_context_node->internal_data.entity_id EQ DTI_ENTITY_TCPIP 	)
          {
//pinghua add one condiction exit to PDP
      dcm_new_state(DCM_CLOSING_CONN, DCM_SUB_WAIT_CGDEACT_CNF);
      dcm_env_ctrl_blk_p->dcm_call_back = dcm_handle_message;
      Is_deactivate_ind_ing = TRUE;
