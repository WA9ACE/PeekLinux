...
{
    SDL_Surface *peek_screen = nsfb->frontend_priv;
    ...
    /* Set palette */
    SDL_SetColors(peek_screen, palette, 0, 256);
...
sdlcopy(nsfb_t *nsfb, nsfb_bbox_t *srcbox, nsfb_bbox_t *dstbox)
{
    ...
    SDL_BlitSurface(peek_screen, &src, peek_screen , &dst);
    ...
    SDL_UpdateRect(peek_screen, dst.x, dst.y, dst.w, dst.h);
...
//    SDL_Surface *peek_screen;
    ...
    peek_screen = SDL_SetVideoMode(nsfb->width,
                                  nsfb->height,
                                  nsfb->bpp,
                                  SDL_SWSURFACE);
    ...
    nsfb->frontend_priv = peek_screen;

    if (nsfb->bpp == 8)
        set_palette(nsfb);

    nsfb->ptr = peek_screen->pixels;
    nsfb->linelen = peek_screen->pitch;
...
static bool peek_input(nsfb_t *nsfb, nsfb_event_t *event, int timeout)
{
    ...
    switch (sdlevent.type) {
        case SDL_KEYDOWN:
            ...
            event->value.keycode = peek_nsfb_map[sdlevent.key.keysym.sym];
            break;
...
static int
peek_cursor(nsfb_t *nsfb, struct nsfb_cursor_s *cursor)
{
#if 0
    SDL_Surface *peek_screen = nsfb->frontend_priv;
    nsfb_bbox_t redraw;
