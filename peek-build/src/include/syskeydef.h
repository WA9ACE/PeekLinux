...
typedef enum
{
   ...
   ETS_KEY_STAR,
   /*
   * do NOT change order or location of above IDs as
   * they are used to index arrays in valhwd.c
   */
   ETS_KEY_TALK,
   ETS_KEY_END_PWR,
   ETS_KEY_BACK,
   ...
   /* Remaining keys are not associated with any hardware, so MUST
   * be placed AFTER BAL_KEY_NUM. Used only by CP Key Press from ETS.
   */
   ETS_KEY_A = 50,
   ETS_KEY_B,
   ETS_KEY_C,
