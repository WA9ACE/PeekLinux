...
bool CMarkup::LoadFromString(char *p)
{
	m_strDoc.copy(p);
	return x_ParseDoc();
}
...
bool CMarkup::ReadTextFile( const char* szFileName, stringC& strDoc, stringC* pstrError, int* pnFlags )
{
...
bool CMarkup::WriteTextFile( const char* szFileName, stringC& strDoc, string* pstrError, int* pnFlags )
{
...
stringC CMarkup::EscapeText( const char* szText, int nFlags )
{
...
char* CMarkup::UnescapeText( const char* szText, int nTextLength /*=-1*/ )
{
	...
	while ( nChar < nTextLength )
	{
		if ( pSource[nChar] == '&' )
		{
			...
			if ( pSource[nChar+1] == '#' )
			{
				...
				if ( cChar == ';' )
				{
					...
					*/
					if ( nUnicode < 0x80 )
						strText.appendChar((char)nUnicode);
					else if ( nUnicode < 0x800 )
					{
						// Convert to 2-byte UTF-8
						strText.appendChar((char)(((nUnicode&0x7c0)>>6) | 0xc0));
						strText.appendChar((char)((nUnicode&0x3f) | 0x80));
					}
					else
					{
						// Convert to 3-byte UTF-8
						strText.appendChar((char)(((nUnicode&0xf000)>>12) | 0xe0));
						strText.appendChar((char)(((nUnicode&0xfc0)>>6) | 0x80));
						strText.appendChar((char)((nUnicode&0x3f) | 0x80));
					}
					if ( nUnicode )
					{
			...
			else // does not start with #
			{
				...
				for ( int nMatch = 0; nMatch < 5; ++nMatch )
				{
						...
						&& strncmp(szaCode[nMatch],&pSource[nChar+1],anCodeLen[nMatch]) == 0 )
					{
						...
						strText.appendChar(szSymbol[nMatch]);
		...
		{
			strText.appendChar(pSource[nChar]);
			++nChar;
		}
	}
...
int CMarkup::FindNode( int nType )
{
	...
	int iPosNew = m_iPos;
	TokenPos token( m_strDoc.c_str(), m_nFlags );
	NodePos node;
...
stringC CMarkup::GetTagName() const
{
...
/*stringC CMarkup::GetAttribName( int n ) const
{
...
stringC CMarkup::x_GetLastError()
{
	stringC strError = strerror(errno);
	for ( int nChar=0; nChar<(int)strError.size(); ++nChar )
		if ( strError[nChar] == '\r' || strError[nChar] == '\n' )
		{
			strError.reserve(nChar ); // no trailing newline
			break;
		}
	return strError;
}
...
bool CMarkup::x_ParseDoc()
{
	...
	if ( m_strDoc.size() )
	{
		...
		if ( iPos > 0 )
		{
			...
			if ( m_aPos[iPos].iElemNext )
				m_strError.copy("Root element has sibling");
		}
		else
			m_strError.copy("No root element");
	}
	else
		m_strError.copy("Empty document");
	...
	{
		if ( m_strError.empty() )
			m_strError.copy(strResult);
		else
			m_strError.copy(strResult);
			m_strError.append(", ");
			m_strError.append(m_strError);
	}
...
int CMarkup::x_ParseElem( int iPosParent, TokenPos& token )
{
	...
	while ( 1 )
	{
		...
		else if ( nTypeFound == 0 ) // end tag
		{
			nMatchDepth = nDepth;
			iPosMatch = iPos;
			while ( nMatchDepth && ! token.Match(aNodes.At(nMatchDepth-1).strMeta.c_str()) )
			{
				/*
		...
		else if ( nTypeFound == -1 )
		{
			...
			m_aPos[iPos].nFlags |= MNF_ILLDATA;
			if ( m_strError.empty() )
				m_strError.copy(aNodes.Top().strMeta);
		}
				...
				if ( m_strError.empty() )
				{
					if ( nTypeFound == 0 )
					{
						...
						sprintf( szError, "End tag '%s' at offset %d does not match start tag '%s' at offset %d",
							x_GetToken(token), token.nL-1, aNodes.Top().strMeta.c_str(), pElem->nStart );
						m_strError.copy(szError);
						delete [] szError;
					}
					else
...
bool CMarkup::x_FindName( TokenPos& token )
{
...
char* CMarkup::x_GetToken( const TokenPos& token )
{
	...
	if ( token.nL > token.nR )
		return "";
	static stringC strToken( &token.szDoc[token.nL], token.Length() );
...
int CMarkup::x_ParseNode( TokenPos& token, NodePos& node )
{
...
/*stringC CMarkup::x_GetPath( int iPos ) const
{
	stringC strPath;
	while ( iPos )
	{
		stringC strTagName = x_GetTagName( iPos );
...
stringC CMarkup::x_GetTagName( int iPos ) const
{
...
bool CMarkup::x_FindAttrib( CMarkup::TokenPos& token, const char* szAttrib, int n )
{
...
stringC CMarkup::x_GetAttrib( int iPos, const char* szAttrib ) const
{
...
bool CMarkup::x_CreateNode( stringC& strNode, int nNodeType, const char* szText )
{
...
stringC CMarkup::x_EncodeCDATASection( const char* szData )
{
	// Split CDATA Sections if there are any end delimiters
	stringC strData = "<![CDATA[";
	const char* pszNextStart = szData;
	const char* pszEnd = strstr( szData, "]]>" );
	while ( pszEnd )
	{
		strData += stringC( pszNextStart, (int)(pszEnd - pszNextStart) );
		strData += "]]]]><![CDATA[>";
...
*/
char* CMarkup::x_GetData( int iPos ) const
{
	if ( iPos == m_iPos && m_nNodeLength )
	{
		...
		if ( pszTag && ((int)(pszTag-pszContent) < nContentLen) )
		{
			...
			while ( token.nNext < nStartContent + nContentLen )
			{
				...
				if ( node.nNodeType == MNT_TEXT )
					strData.append(UnescapeText( &token.szDoc[node.nStart], node.nLength ));
				else if ( node.nNodeType == MNT_CDATA_SECTION )
					{
						char *p = NULL;
						p = m_strDoc.substr( node.nStart+9, node.nLength-12 );
						strData.append(p);
						if(p)
							delete p;
					}
			...
			//strData.copy
			return (UnescapeText( &(m_strDoc.c_str())[nStartContent], nContentLen ) );
	}
	return strData.c_str();
}
/*
stringC CMarkup::x_GetElemContent( int iPos ) const
{
...
void CMarkup::x_DocChange( int nLeft, int nReplace, const stringC& strInsert )
{
...
int CMarkup::x_InsertNew( int iPosParent, int& iPosRel, CMarkup::NodePos& node )
{
	...
	if ( bEmptyParentTag )
	{
		stringC strTagName = x_GetTagName( iPosParent );
		stringC strFormat;
		if ( node.nFlags & MNF_WITHNOLINES )
			strFormat = ">";
		else
...
stringC CMarkup::x_GetSubDoc( int iPos ) const
{
	if ( iPos )
	{
...
int CMarkup::x_RemoveElem( int iPos )
{
	...
	x_DocChange( m_aPos[iPos].nStart, nLen, stringC() );
...
void CMarkup::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
	...
	// Remove node from document
 	x_DocChange( nNodeOffset, nNodeLength, stringC() );
