                    ...
                    int flags)
{
    ...
    mode = 1;//bank_number(ARM_CPU_MODE_SVC);
    addr = 0x006D75E4;//0x006F0890; //env->banked_r13[mode];
   
    cpu_fprintf(f, "Stack: SP: 0x%08x\n", addr);
    cpu_fprintf(f, "--------------------------------\n");

    for(i=0;i<0x100;i++) {
	cpu_physical_memory_rw(addr, buf, sizeof(buf), 0);
    	cpu_fprintf(f, "0x%08x - 0x%08x\n", addr, (uint32_t)ldl_raw(buf));
	addr -=4;
    }
